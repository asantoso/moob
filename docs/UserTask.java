...
public abstract class UserTask<Params, Progress, Result> {
    ...
    private static final ThreadFactory sThreadFactory = new ThreadFactory() {
        ...
        static final String NAMEBASE = "UserTask";
    ...
    public static void initStatics(){
    	...
    	if(sPendingQueue == null){
    		sPendingQueue = new LinkedBlockingQueue<Runnable>(MAXIMUM_POOL_SIZE * 2);
    	}
    	
    	if(sExecutor == null){
    		sExecutor = new ThreadPoolExecutor(CORE_POOL_SIZE,MAXIMUM_POOL_SIZE, KEEP_ALIVE, KEEP_ALIVE_UNIT, sWorkQueue, sThreadFactory);
    ...
    static UserTaskExecutionScope sExecScope;
    
    static boolean isStaticsInitialized = false;
    ...
    public void doInit(){
        ...
        mFuture = new FutureTask<Result>(mWorker) {
            ...
            protected void done() {
                ...
                int poolSize = sExecutor.getPoolSize();
                int corePoolSize = sExecutor.getCorePoolSize();
                long taskCompleteCount = sExecutor.getCompletedTaskCount();
                long taskCount = sExecutor.getTaskCount();
                int maxPoolSize = sExecutor.getMaximumPoolSize();
                int largestPoolSize = sExecutor.getLargestPoolSize();
                Logger.l(Logger.DEBUG, LOG_TAG, threadName+" [FutureTask][done()] corePoolSize:"+corePoolSize+", poolSize: "+poolSize+", taskCompleted: "+taskCompleteCount+", task: "+taskCount+
                		", maxPoolSize:"+maxPoolSize+
                		", largestPoolSize:"+largestPoolSize
                		
                );
                try{                	
                	sExecutor.execute(sPendingQueue.remove());
                	Logger.l(Logger.DEBUG, LOG_TAG, threadName+" [FutureTask][done()] adding one pending task to be execution queue. done.");
                }catch(Exception e){
    ...
    static RejectedExecutionHandler mRejectedHandler =  new RejectedExecutionHandler() {			
		@Override
		public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
			...
			if(!isTerminating){				
				try{					
					sPendingQueue.put(r);
					Logger.l(Logger.WARN, LOG_TAG, "[rejectedExecution()] task is successfully added to pending task queue. ");
				}catch(InterruptedException e){
					Logger.l(Logger.ERROR, LOG_TAG, "[rejectedExecution()] task can not be added to pending task queue. ");
				}
			}
		}
    };
