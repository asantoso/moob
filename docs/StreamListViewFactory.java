...
public class StreamListViewFactory extends BaseExpandableListViewFactory<Cursor,StreamListViewFactory.PostQuickInfo> {
	...
	SoftHashMap<String, PostQuickInfo> mPQI_Registry = new SoftHashMap<String, PostQuickInfo>(100, 1.5f);
	...
	@Override
	public SoftHashMap<String, PostQuickInfo> getMemoryCache() {
		return mPQI_Registry;
		
	}	
	
	/**
	 * Mark rows in cache as dirty and needs to updated from an underlying cursor.
	 * @param ids the id's of the rows to be marked as dirty
	 */
	public void markGroupAsDirty(String[] ids){
		if(ids == null){
			Log.d(LOG_TAG,"markAsDirty()"+(ids == null));
			return;
		}
				
		for(int i=0,num=ids.length; i<num; i++){
			if(ids[i]!=null){
				Log.d(LOG_TAG,"markAsDirty()"+ids[i]);
	...
	public View createGroupView(
		...
		if (convertView != null && convertView.getTag() != null ) {
			tag = (GroupHolder) convertView.getTag();
		...
		// A solution when the user presses the back button while the list is scrolling really fast
		try{
			//int numColumns = ds.getColumnCount();
			//Logger.l(Logger.DEBUG, LOG_TAG, "[createGroupView()] num columns in the cursor: "+numColumns);
			post_id = ds.getString(Stream.col_post_id + startStreamTableIndex);
		}
		catch(IllegalStateException e){
			Logger.l(Logger.ERROR,LOG_TAG,"[createGroupView()] illegal state. "+e.getMessage());
			return convertView;
		}catch(CursorIndexOutOfBoundsException e){
			Logger.l(Logger.ERROR,LOG_TAG,"[createGroupView()] cursor index out of bounds. "+e.getMessage());
			return convertView;
		}catch(StaleDataException e){
			Logger.l(Logger.ERROR,LOG_TAG,"[createGroupView()] stale data. "+e.getMessage());
			return convertView;
		}
		...
		if (existingPQI != null && !existingPQI.isDirty) {
			...
			for (int i = 0; i < existingPQI.numMediaImages; i++) {
				...
				Drawable d = App.mImageUrlLoader.loadImage(existingPQI.mediaImagesSrcs[i], true);
			...
			for (int i = existingPQI.numMediaImages;
					i < GroupHolder.TOTAL_MEDIAIMAGES; 
					i ++) {
			...
			for (int i = 0; i < GroupHolder.TOTAL_MEDIAIMAGES; i++) {
				tag.mediaimages[i].setVisibility(View.GONE);
			}
			...
			existingPQI.actor_id = actorId;
			existingPQI.post_id = post_id;		
			existingPQI.name = actorName;
			existingPQI.target_id = targetId;
		...
		if (existingPQI.profilePictureUrl == null || existingPQI.profilePictureUrl.length() == 0) {				
			...
			// image from cache.
			Drawable img = App.mImageUrlLoader.loadImage(existingPQI.profilePictureUrl, true);
	...
	public boolean process(Cursor ds, int position){
		...
		long actor_id = ds.getLong(Stream.col_actor_id+ApplicationDBHelper.START_STREAM_INDEX_GETALL_STREAMPOSTS_AND_USERBASIC);
		long target_id = ds.getLong(Stream.col_target_id+ApplicationDBHelper.START_STREAM_INDEX_GETALL_STREAMPOSTS_AND_USERBASIC);
		
		PostQuickInfo pqi = mPQI_Registry.get(post_id);
		if(pqi == null){
			pqi = new PostQuickInfo();
		}
		pqi.target_id = target_id;
		pqi.actor_id = actor_id;
		pqi.post_id = post_id;
	...
	*/
			...
			try {
				...
				case Attachment.ATTACHMENT_TYPE_MEDIALINK: {
					...
					try {
						...
						App.mImageUrlLoader.loadImage(src, false);
					} catch (JSONException e) {
						//e.printStackTrace();
					}
				...
				case Attachment.ATTACHMENT_TYPE_MEDIAIMAGE: {
					...
					for (int i = 0; i < pqi.numMediaImages; i++) {
						try {
							...
							App.mImageUrlLoader.loadImage(pqi.mediaImagesSrcs[i], false);
						} catch (JSONException e) {
							//e.printStackTrace();
						}
					}
	...
	class BGLoader extends UserTask<AsyncLoadData, Byte, Short> {
		...
		protected Short doInBackground(AsyncLoadData... params) {
			...
			if (mPostData.isLoadProfileImage) {
				...
				mProfileImageDrawable = App.mImageUrlLoader.loadImage(imgUrl, false);
			...
			if (mPostData.isLoadMediaImages) {
				...
				for (int i = 0; i < mPostData.mPQI.numMediaImages; i++) {
					mMediaPhotosDrawables[i] = App.mImageUrlLoader.loadImage(
